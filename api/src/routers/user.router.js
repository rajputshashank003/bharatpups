...
router.get("/send_email_verification/:id", async (req, res) => {
    try {
        ...
        sendMail(userData.email, "Mail Verification", verify_email_content(userData.name, req.params.id));
...
const generateTokenResponse = user => {
    const token = jwt.sign({
        id: user?.id, email: user?.email, isAdmin: user?.isAdmin, name: user?.name, image: user?.image
    },
