        ...
        >
            <div className='w-full bg-neutral-800 h-[300px] p-[5px] rounded-[8px] gap-2 grid grid-rows-5' >
                <div className=' relative row-span-3 w-full h-full shadow-[0px_0px_1px] shadow-neutral-500 rounded-b-none rounded-[6px] '>
                    ...
                    <div
                        className="h-full relative cursor-pointer duration-200 rounded-[6px] rounded-b-none overflow-hidden bg-cover bg-center"
                        style={{ backgroundImage: `url(${add_quality_to_cloudinary(food.image)})` }}
                        onClick={() => navigate(`/dog/${food._id}`)}
                    >
                        <div className="absolute inset-0 backdrop-blur-[6px]"></div>
                        <img
                            src={add_quality_to_cloudinary(food.image)}
                            alt={`Image of ${food?.breed}`}
                            className="h-full w-full relative z-[1] object-contain cursor-pointer"
                        />
                    </div>
